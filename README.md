...
<dl>
    <dt>未完成事項 (テストID:11 支払い方法選択機能)</dt>
    <dd>今回の模擬試験の全てのテストの仕様を教材にあるデフォルトのテスト手法Featureテストで作ることを想定していましたが、支払方法の即時反映機能をJavaScriptを使って実現したため、Featureテストを使って即時反映をテストするコードが書けませんでした。Featureテストを使ってみてFeatureテストだけでは今回のテストの仕様を真に正しく判定できないと感じましたが、どこまでテストの質を求められているのか判断しきれず、時間的にもBrowserテストなどのテストに移行する時間がもてませんでした。</dd>
</dl>

<dl>
    <dt>応用要件の未着手事項</dt>
    <dd>1. メール認証</dd>
    <dd>2. stripeを使った決済処理</dd>
    <dd>試験後の自主学習とします。</dd>
</dl>

<dl>
    <dt>最後に</dt>
    <dd>模擬試験を通してこれまで学習が不足していた部分をたくさん学ぶことができました。この場を借りて御礼申し上げます。</dd>
</dl>
